...
def test_KS_differentiability():
    excitation_length=0.005
    n_examples = 3
    batch_size = 1
    sr = 16000
    example_length = 2 # sec
    # Number of sources in the mix
    J = 2
    # The FFT hop size is the audio frame length
    fft_hop_size = 256
    ...
    print("Test differentiability with clear_state()")
    for example in range(n_examples):
        print("Calculate example", example)
        f0_in = f0_hz[:, :, example * N : example * N + N]
        fc_in = fc[:, :, example * N : example * N + N]
        ...
        # Clear state and disable gradient tracking.
        ks.clear_state()
    
    print("Test differentiability with detach()")
    for example in range(n_examples):
        print("Calculate example", example)
        f0_in = f0_hz[:, :, example * N : example * N + N]
        fc_in = fc[:, :, example * N : example * N + N]
...
if __name__ == "__main__":
    #test_KS_synthetic_input()
    test_KS_differentiability()
